...
typedef struct {
    ...
    */
    PyObject_HEAD
    Py_ssize_t length;          /* Number of code points in the string */ // 字符串中码位的数量
    Py_hash_t hash;             /* Hash value; -1 if not set */ // 缓存了字符串对象的hash值
    struct {
        /*
         ...
         */
        unsigned int interned:2; // 标记该对象是否已经通过intern机制
        /* Character size:
    ...
    wchar_t *wstr;              /* wchar_t representation (null-terminated) */ // 真实的字符串值
} PyASCIIObject;
...
/* Interning state. */
#define SSTATE_NOT_INTERNED 0 // 没有被intern机制处理
#define SSTATE_INTERNED_MORTAL 1 // 被intern机制处理，但会随引用计数被回收
#define SSTATE_INTERNED_IMMORTAL 2 // 被intern机制处理，但对象不可销毁，会与Python解释器同在
